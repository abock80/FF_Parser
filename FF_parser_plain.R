...
EXPdir<-"d:/abock/CDOT_Flood_Freq"
setwd(EXPdir)
#**********************************************************
...
Sdf[,1]<-STAIDs
colnames(Sdf)<-c("STAID","Skew","StandDev","RegSkew","SysPeaks")
EPdf[,1]<-STAIDs
Estdf[,1]<-STAIDs
Kdf[,1]<-STAIDs
write.table(Sdf,"Stats.txt",sep=" ",col.names=colnames(Sdf),quote=FALSE,row.names=FALSE)
write.table(EPdf,"ExProb.txt",sep=" ",col.names=FALSE,quote=FALSE,row.names=FALSE)
write.table(Estdf,"Est.txt",sep=" ",col.names=FALSE,quote=FALSE,row.names=FALSE)
